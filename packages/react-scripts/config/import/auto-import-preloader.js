...
module.exports = function(source) {
  try {
    ...
    const aliases = [
      { name: 'pmt-modules/', directory: './pmt-react-sdk/packages/modules/src/' },
      { name: 'pmt-ui/', directory: './pmt-react-sdk/packages/ui/src/' },
      { name: 'pmt-utils/', directory: './pmt-react-sdk/packages/utils/src/' },
    ]
    ...
    autoImportDefinitions.forEach(autoImport => {
      ...
      aliases.forEach(alias => {
        from = from.replace(alias.name, alias.directory)
      })

      // should we add the import?
      if (isNotDefinitionFile(from, currentFileName) &&
...
function containsImportUsage(autoImport, fileContent) {
  ...
  // find the function name on raw string, to markeravoid building ast if there is no trace of the
  // name.
